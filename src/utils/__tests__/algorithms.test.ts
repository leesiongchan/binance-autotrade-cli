...
describe("findArbitrage", () => {
  ...
  it("should return the result", async () => {
    ...
    const arbitrageResult = findArbitrage({
      balances,
      bollingerBands,
      orderBooks,
      prices,
    ...
    const abBidAmount = arbitrageResult?.[0].bid.quantity;
    const acAskAmount = arbitrageResult?.[1].ask.quantity;
    const cbAskAmount = arbitrageResult?.[2].ask.quantity;

    expect(abBidAmount).toBe(0.7092198581560284);
    expect(acAskAmount).toBe(0.7092198581560284);
    expect(cbAskAmount).toBe(5014.184397163121);
    expect(arbitrageResult).not.toBeNull();
  });
});
